...
function round(state, roundConstant) {
    // θ step
    const tempC = [];
    for (let x = 0; x < 5; ++x) {
    ...
    const tempD = [];
    for (let x = 0; x < 5; ++x) {
        tempD[x] = tempC[(x + 4) % 5] ^ utilities_1.leftCircularShift64(tempC[(x + 1) % 5], 1n);
    }
    for (let x = 0; x < 5; ++x) {
        for (let y = 0; y < 5; ++y) {
            state[x][y] ^= tempD[x];
    ...
    // ρ and π steps
    const tempB = [[0n, 0n, 0n, 0n, 0n], [0n, 0n, 0n, 0n, 0n], [0n, 0n, 0n, 0n, 0n], [0n, 0n, 0n, 0n, 0n], [0n, 0n, 0n, 0n, 0n]];
    for (let x = 0; x < 5; ++x) {
        for (let y = 0; y < 5; ++y) {
            tempB[y][(2 * x + 3 * y) % 5] = utilities_1.leftCircularShift64(state[x][y], rotationOffests[x][y]);
        }
    }
    // χ step
    for (let x = 0; x < 5; ++x) {
...
async function hash(message) {
    utilities_1.validateByteArray(message);
    // padding
