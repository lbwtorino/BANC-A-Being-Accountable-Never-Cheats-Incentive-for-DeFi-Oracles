 ...
 */
async function getErrorReason(words) {
...
async function toSeed(words, passphrase) {
    ...
    return utilities_1.bytesToBigint(new Uint8Array(derivedKey));
}
 ...
 */
async function generateRandom(entropyLengthInBits) {
 ...
 */
async function generateFromEntropy(entropy) {
    if (entropy.length % 4)
    ...
    const wordBits = [...utilities_1.toBitArray(entropy), ...entropyChecksum];
    const wordIndexes = [];
    for (let i = 0; i < wordBits.length; i += 11) {
        let index = utilities_1.bitArrayToNumber(wordBits.slice(i, i + 11));
        wordIndexes[i / 11] = index;
    }
    const words = wordIndexes.map(wordNumber => englishWordList[wordNumber]);
    return words;
}
exports.generateFromEntropy = generateFromEntropy;
function toEntropy(/*words*/ _) {
...
async function entropyToChecksum(entropy) {
    ...
    return utilities_1.getFirstBits(entropyHash, checksumLengthInBits);
}
