import { getFirstBits, toBitArray, bitArrayToNumber, bytesToBigint } from "./utilities.js";
 ...
 */
export async function getErrorReason(words) {
    if (typeof words === 'string')
        words = words.trim().split(' ');
    // validate the length
    if (words.length !== 12 && words.length !== 15 && words.length !== 18 && words.length !== 21 && words.length !== 24) {
        return `Expected 12, 15, 18, 21, or 24 words but got ${words.length} word${words.length === 1 ? '' : 's'}.`;
    }
    // validate all of the words are in the wordlist
    for (const word of words) {
        if (!englishWordList.includes(word)) {
            return `'${word}' is not in the English words list.`;
        }
    }
    // TODO: validate checksum
    // const entropy = toEntropy(words)
    // const expectedChecksum = bitArrayToNumber(await entropyToChecksum(entropy))
    // const lastWord = words[words.length - 1]
    // const lastWordIndex = englishWordList.lastIndexOf(lastWord)
    // const checksumLengthInBits = words.length / 3
    // const actualChecksum = lastWordIndex & ((1 << checksumLengthInBits) - 1)
    // if (expectedChecksum !== actualChecksum) return `Checksum doesn't match expected.`
    return null;
}
/**
...
export async function toSeed(words, passphrase) {
    ...
    if (passphrase === undefined)
        passphrase = '';
    const invalidMnemonicReason = await getErrorReason(words);
    if (invalidMnemonicReason)
        throw new Error(`Invalid mnemonic.  ${invalidMnemonicReason}`);
    ...
    return bytesToBigint(new Uint8Array(derivedKey));
}
/**
 ...
 */
export async function generateRandom(entropyLengthInBits) {
    const entropy = crypto.getRandomValues(new Uint8Array(entropyLengthInBits / 8));
    return generateFromEntropy(entropy);
}
/**
 ...
 */
export async function generateFromEntropy(entropy) {
    if (entropy.length % 4)
        throw new Error(`Entropy must be a multiple of 4 bytes in length.`);
    if (entropy.length < 16)
        throw new Error(`Must provide more than 16 bytes of entropy.`);
    if (entropy.length > 32)
        throw new Error(`Must provide less than 32 bytes of entropy.`);
    const entropyChecksum = await entropyToChecksum(entropy);
    const wordBits = [...toBitArray(entropy), ...entropyChecksum];
    const wordIndexes = [];
    for (let i = 0; i < wordBits.length; i += 11) {
        let index = bitArrayToNumber(wordBits.slice(i, i + 11));
        wordIndexes[i / 11] = index;
    }
    const words = wordIndexes.map(wordNumber => englishWordList[wordNumber]);
    return words;
}
export function toEntropy(/*words*/ _) {
    // TODO: turn words into array of bits
    // TODO: split checksum bits from entropy bits
    // TODO: return entropy bits
    throw new Error(`Not implemented yet.`);
}
async function entropyToChecksum(entropy) {
    const checksumLengthInBits = entropy.length / 4;
    const entropyHash = new Uint8Array(await crypto.subtle.digest('SHA-256', entropy));
    return getFirstBits(entropyHash, checksumLengthInBits);
}
